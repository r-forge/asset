...
p.dlm.ls <- function(t.vec, k, search, side, sizes=rep(1, k), sub.def=NULL, sub.args=NULL, wt.def=NULL, wt.args=NULL)
{
	dlm.pval.ls(t.vec, k, search, side, sizes=sizes, sub.def=sub.def, sub.args=sub.args)
}

dlm.pval.ls <- function(t.vec, k, search, side, sizes = rep(1, k), sub.def=NULL, sub.args=NULL, wt.def=NULL, wt.args=NULL)
{
	if(k == 1) return(t.vec)
...
#		Valid neighbors
		...
		sign.vec <- apply(nn.x[, nn.sub], 2, function(set1) { v <- (set1 - set) ; sum(v)})
...
#		Add a term to points with integral still below 1
		...
		if(length(t.sub) > 0 && search < 2)
		{
			int <- sapply(t.sub, function(j) qxd.prod.int(low[j], high[j], search, chisq.cprob, side=side
										, sign.vec = sign.vec))
			ss[t.sub] <- ss[t.sub] + NXX * int	
		}
...
chisq.cprob <- function(zv, search, side, sign.vec)
{
	npts <- length(zv)
	kk <- length(sign.vec)
	ss <- matrix(sum(sign.vec < 0), npts, kk)
	cprob <- matrix(1, npts, kk)
	zv.mat <- matrix(zv, npts, kk)
	pos <- (sign.vec > 0)
	neg <- (sign.vec < 0)
	if(any(pos)) cprob[, pos] <- 1 - exp(-0.5 * (qchisq(pnorm(zv.mat[, pos]), df = 2 * (ss[, pos] + 1)) - qchisq(pnorm(zv.mat[, pos]), df = 2 * ss[, pos])))
	if(any(neg)) cprob[, neg] <- exp(0.5 * (qchisq(pnorm(zv.mat[, neg]), df = 2 * (ss[, neg] - 1)) + qchisq(pnorm(zv.mat[, neg]), df = 2 * ss[, neg])))
	cprob[is.nan(cprob)] <- 1
	cprob[cprob > 1] <- 1
	cprob[cprob < 0] <- 0
	ret <- apply(cprob, 1, prod)
	ret
}
...
coef.meta <- function(x1, X2, k, ncase, ncntl, rmat=NULL, cor.numr=FALSE)
{
	...
	sxx <- diag(t(matrix(rneff[xcom, ], ncol = nsnp)) %*% matrix(rmat[xcom, xcom], ncom, ncom) %*% matrix(rneff[xcom, ], ncol = nsnp))
	sxd <- (matrix(rmat[xdel, xcom], ndel, ncom) %*% matrix(rneff[xcom, ], ncol = nsnp)) * (sgn * matrix(rneff[xdel, ], ncol=nsnp))
	sdd <- matrix(rneff[xdel, ] * rneff[xdel, ], ncol=nsnp)
...
#			print(c(1111, p1, p2))
		...
		if(search == 2)
		{
			...
			Z2 <- tubeSim(x = (x), t0 = zeros, nsamp = nsamp, rmat = rmat, neff = neff)
