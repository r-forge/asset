...
#		print(c(i, set, ztmp, opt.z))
		...
		if(npos > 0)
		{
			opt.z[snp.sub[pos]] <- ztmp[pos]
			opt.s[snp.sub[pos], ] <- matrix(set, npos, k, byrow=TRUE)
			nopt[snp.sub[pos]] <- nsub
		}
...
h.forest <- function(k, snp.var, t.lab, rlist, res, side, level, p.adj, digits)
{
	...
	CI0.str <- paste("(", format(CI0[1, 2], digits=digits), ", ", format(CI0[1, 3], digits=digits), ")", sep="")
	CI1.str <- paste("(", format(CI1[1, 2], digits=digits), ", ", format(CI1[1, 3], digits=digits), ")", sep="")
	if(side == 1)
	{
		CI2.str <- paste("(", format(CI2[1, 2], digits=digits), ", ", format(CI2[1, 3], digits=digits), ")", sep="")
	} else
	{
		CI2.1.str <- paste("(", format(CI2.1[1, 2], digits=digits), ", ", format(CI2.1[1, 3], digits=digits), ")", sep="")
		CI2.2.str <- paste("(", format(CI2.2[1, 2], digits=digits), ", ", format(CI2.2[1, 3], digits=digits), ")", sep="")
	}
	
	CI <- getCI(res$beta, res$sd, level = level)
	CI.str <- apply(data.matrix(CI), 1, function(x)
					{
					paste("(", format(x[2], digits=digits), ", ", format(x[3], digits=digits), ")", sep="")
					})
	if(side == 1)
	{
	...
	tabletext <- cbind(pheno.col, c("OR", round(mid, digits=digits))
					   , rep("", length(mid) + 1)
					   , c(paste(round(1 - level, dig=3), "% CI", sep=""), CI.vec)
					   , rep("", length(mid) + 1)
